"""
Tests for magnetohydrodynamics.stability.velikhov_instability.
...
class TestAlpha:
    def test_matches_hand_derivation(self):
        ...
        assert alpha(6000.0, 3.894, 0.5) == pytest.approx(expected, rel=1e-12)

    def test_finite_as_fI_approaches_zero(self):
        # f_I=0: (2-f_I)/(1-f_I) = 2, so alpha is finite and nonzero (weakly ionised seed).
        result = alpha(6000.0, 3.894, 0.0)
        assert np.isfinite(result)
        assert result > 0.0

    def test_diverges_as_fI_approaches_one(self):
        with np.errstate(divide="ignore"):
            result = alpha(6000.0, 3.894, 1.0)
...
class TestCriticalHallParameterExact:
    def test_matches_hand_derivation(self):
        ...
        assert critical_hall_parameter(6000.0, 2000.0, 3.894, 0.5) == pytest.approx(expected, rel=1e-12)

    def test_freidberg_section_7_3_worked_example(self):
        ...
        numeric procedure exactly (not just this module's formula) still lands ~8.6% off
        the stated 8.249. Do not tighten this to chase bit-for-bit reproduction of the
        paper; the point is "right ballpark, no silent regression," not exact replication.
        """
        Te = 4223.0
        Tp = Te / 8.783
        f_I = 1.0 - 0.01679
        result = critical_hall_parameter(Te, Tp, 3.894, f_I)
        assert result == pytest.approx(8.249, rel=0.10)

    def test_diverges_as_temperatures_converge(self):
        """Delta_T -> 0 (Te == Tp): no electron/gas temperature difference to drive the
        mode -- unconditionally stable, beta_crit -> +inf."""
        with np.errstate(divide="ignore", invalid="ignore"):
            result = critical_hall_parameter(2000.0, 2000.0, 3.894, 0.5)
        assert np.isinf(result)
    ...
    def test_diverges_as_fI_approaches_one(self):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            result = critical_hall_parameter(6000.0, 2000.0, 3.894, 1.0)
        assert np.isinf(result)

    def test_no_unhandled_warnings_at_divergent_inputs(self, recwarn):
        critical_hall_parameter(2000.0, 2000.0, 3.894, 1.0)
        assert len(recwarn) == 0

    def test_array_broadcast_matches_scalar_loop(self):
        Te = np.linspace(3000.0, 8000.0, 5)
        Tp = 2000.0
        f_I = np.linspace(0.1, 0.9, 5)
        result = critical_hall_parameter(Te, Tp, 3.894, f_I)
        expected = np.array([
            critical_hall_parameter(float(te), Tp, 3.894, float(fi)) for te, fi in zip(Te, f_I)
        ])
...
class TestCriticalHallParameterAsymptotic:
    def test_matches_hand_derivation(self):
        ...
        assert critical_hall_parameter_asymptotic(6000.0, 3.894, 0.9) == pytest.approx(expected, rel=1e-12)

    def test_diverges_as_fI_approaches_one(self):
        with np.errstate(divide="ignore"):
            result = critical_hall_parameter_asymptotic(6000.0, 3.894, 1.0)
        assert np.isinf(result)

    def test_array_broadcast_matches_scalar_loop(self):
        f_I = np.linspace(0.9, 0.999, 4)
        result = critical_hall_parameter_asymptotic(6000.0, 3.894, f_I)
        expected = np.array([critical_hall_parameter_asymptotic(6000.0, 3.894, float(fi)) for fi in f_I])
        np.testing.assert_allclose(result, expected, rtol=1e-12)
...
class TestExactVsAsymptoticSelfConsistency:
    ...
    def test_ratio_converges_to_one_in_the_joint_limit(self):
        ...
        for f_I in f_I_values:
            ...
            Tp = Te / (1.0 + delta_T)
            exact = critical_hall_parameter(Te, Tp, E_I, f_I)
            asymptotic = critical_hall_parameter_asymptotic(Te, E_I, f_I)
            deviations.append(abs(exact / asymptotic - 1.0))
...
class TestStabilityHelpers:
    def test_stability_margin_greater_than_one_when_stable(self):
        beta_crit = critical_hall_parameter(6000.0, 2000.0, 3.894, 0.5)
        assert stability_margin(1.0, beta_crit) > 1.0
        assert stability_margin(2.0, beta_crit) < 1.0

    def test_is_stable_matches_margin(self):
        beta_crit = critical_hall_parameter(6000.0, 2000.0, 3.894, 0.5)
        assert bool(is_stable(1.0, beta_crit)) is True
        assert bool(is_stable(2.0, beta_crit)) is False


# --- Plasma-based wrapper pass-through tests --------------------------------------------
...
class TestPlasmaWrappers:
    def test_plasma_alpha_is_a_pure_pass_through(self, plasma):
        expected = alpha(plasma.electron_temperature, plasma.seed_type.ionization_potential, plasma.ionization_fraction)
        assert plasma_alpha(plasma) == pytest.approx(expected, rel=1e-12)

    def test_plasma_critical_hall_parameter_is_a_pure_pass_through(self, plasma):
        expected = critical_hall_parameter(
            plasma.electron_temperature, plasma.gas_temperature,
            plasma.seed_type.ionization_potential, plasma.ionization_fraction,
        )
        assert plasma_critical_hall_parameter(plasma) == pytest.approx(expected, rel=1e-12)

    def test_plasma_critical_hall_parameter_asymptotic_is_a_pure_pass_through(self, plasma):
        expected = critical_hall_parameter_asymptotic(
            plasma.electron_temperature, plasma.seed_type.ionization_potential, plasma.ionization_fraction,
        )
        assert plasma_critical_hall_parameter_asymptotic(plasma) == pytest.approx(expected, rel=1e-12)

    def test_plasma_stability_margin_is_a_pure_pass_through(self, plasma):
        expected = stability_margin(plasma.hall_parameter, plasma_critical_hall_parameter(plasma))
        assert plasma_stability_margin(plasma) == pytest.approx(expected, rel=1e-12)

    def test_is_plasma_stable_matches_margin(self, plasma):
        margin = plasma_stability_margin(plasma)
        assert bool(is_plasma_stable(plasma)) == (margin >= 1.0)
