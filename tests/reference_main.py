"""
Frozen reference implementation of the 1-D linear Hall MHD generator.

This is a byte-for-byte copy of the original single-file `main.py`, kept here
purely as the numerical "ground truth" oracle for the test suite. The
top-level `main.py` has since been migrated to use the `magnetohydrodynamics`
package (see main.py, tests/test_main.py); this file must NOT be edited to
follow that migration -- its entire purpose is to stay exactly as validated
against the literature, so tests can prove the package reproduces it.

CAVEAT: `solve_plasma_properties`'s local Ohm's-law/Saha closure is still a
valid oracle (literature-derived, unchanged -- see tests/test_hall_solver.py's
TestSingleSlice). Its `march_channel`'s axial integration is NOT: it has a
known energy-accounting bug (see Derivation.md's "Correction" note) that
HallSolver.march no longer reproduces on purpose. Don't use march_channel()
as an oracle for full axial profiles -- see tests/test_energy_accounting.py.
"""
import time
